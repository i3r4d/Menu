...
const FlavorGrid = ({ flavors, currentCategoryType = 'E-Liquid', title }: FlavorGridProps) => {
  ...
  return (
    <div className="container mx-auto px-4 w-full max-w-6xl">
      {/* Optional Title */}
      {title && <h2 className="text-xl font-semibold dark:text-darkTextHeading mb-6 px-4 sm:px-6 lg:px-8">{title}</h2>}

      {/* Filter/Sort Controls Section */}
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 sticky top-[140px] sm:top-[140px] bg-background/80 dark:bg-darkBgPrimary/90 backdrop-blur-sm py-4 z-10 border-b dark:border-darkBorder rounded-md"> 
        <div className="flex items-center gap-2">
          <Sheet>
            <SheetTrigger asChild>
              <Button 
                variant="outline" 
                size="sm" 
                className="flex items-center gap-1 font-medium dark:bg-darkFormBg dark:border-darkBorder dark:text-darkTextHeading"
              >
                <SlidersHorizontal size={16} className="dark:text-primaryAccent" />
                <span>Filter</span>
              </Button>
            </SheetTrigger>
            <SheetContent side="left" className="border-r dark:bg-darkBgSecondary dark:border-darkBorder">
              <SheetHeader>
                <SheetTitle className="text-2xl font-semibold dark:text-darkTextHeading">Filter Options</SheetTitle>
              </SheetHeader>
              <div className="py-6 space-y-8 overflow-y-auto h-[calc(100vh-120px)]">
                {/* Bottle Sizes Filter UI */}
                {availableBottleSizes.length > 0 && (
                    <div>
                    <h3 className="font-semibold mb-3 text-lg dark:text-darkTextHeading">Bottle Sizes</h3>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div key={`filter-size-${size}`} className="flex items-center space-x-2">
                            ...
                            />
                            <Label htmlFor={`filter-size-${size}`} className="font-medium dark:text-darkTextMuted">{size}</Label>
                        </div>
                        ))}
                    </div>
                    </div>
                ...
                {availableNicLevels.length > 0 && (
                    <div>
                    <h3 className="font-semibold mb-3 text-lg dark:text-darkTextHeading">Nicotine Levels</h3>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div key={`filter-nic-${level}`} className="flex items-center space-x-2">
                            ...
                            className="dark:border-darkBorder dark:data-[state=checked]:bg-primaryAccent dark:data-[state=checked]:border-primaryAccent"
                            />
                            <Label htmlFor={`filter-nic-${level}`} className="font-medium dark:text-darkTextMuted">{level}mg</Label>
                        </div>
                        ))}
                    </div>
                    </div>
                ...
                {overallMaxPrice > 0 && (
                    ...
                    <div className="flex justify-between items-center mb-3">
                        <h3 className="font-semibold text-lg dark:text-darkTextHeading">Price Range</h3>
                        <span className="text-sm font-medium bg-primary/10 dark:bg-primaryAccent/10 px-3 py-1 rounded-full dark:text-darkTextMuted">
                ...
                {/* VG/PG Ratio Filter UI */}
                 {availableVgPgRatios.length > 0 && (
                    <div>
                    <h3 className="font-semibold mb-3 text-lg dark:text-darkTextHeading">VG/PG Ratio</h3>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div key={`filter-ratio-${ratio}`} className="flex items-center space-x-2">
                            ...
                            className="dark:border-darkBorder dark:data-[state=checked]:bg-primaryAccent dark:data-[state=checked]:border-primaryAccent"
                            />
                            <Label htmlFor={`filter-ratio-${ratio}`} className="font-medium dark:text-darkTextMuted">{ratio}</Label>
                        </div>
                        ))}
                    </div>
                    </div>
                ...
                <div className="flex justify-between pt-4 border-t dark:border-darkBorder mt-8">
                  <Button 
                    ...
                    className="font-medium dark:bg-darkFormBg dark:border-darkBorder dark:text-darkTextHeading"
                  >
                    Reset Filters
                  </Button>
                </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <span>Sort</span>
                <ChevronDown size={16} className="dark:text-primaryAccent" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="bg-background/95 dark:bg-darkBgSecondary backdrop-blur-sm border dark:border-darkBorder rounded-xl shadow-lg p-2">
              <DropdownMenuItem onClick={() => handleSortChange('name-asc')} className="cursor-pointer rounded-md mb-1 dark:text-darkTextHeading dark:focus:bg-darkFormBg dark:focus:text-primaryAccent">Name (A-Z)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleSortChange('name-desc')} className="cursor-pointer rounded-md mb-1 dark:text-darkTextHeading dark:focus:bg-darkFormBg dark:focus:text-primaryAccent">Name (Z-A)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleSortChange('price-asc')} className="cursor-pointer rounded-md mb-1 dark:text-darkTextHeading dark:focus:bg-darkFormBg dark:focus:text-primaryAccent">Price (Low-High)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleSortChange('price-desc')} className="cursor-pointer rounded-md mb-1 dark:text-darkTextHeading dark:focus:bg-darkFormBg dark:focus:text-primaryAccent">Price (High-Low)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleSortChange('newest')} className="cursor-pointer rounded-md dark:text-darkTextHeading dark:focus:bg-darkFormBg dark:focus:text-primaryAccent">Newest Added</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>

        {/* Flavor count */}
        <div className="text-sm text-muted-foreground dark:text-darkTextMuted font-medium px-3 py-1 bg-background/50 dark:bg-darkFormBg dark:border-darkBorder border rounded-full">
      ...
      {filteredFlavors.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center justify-center text-center mt-16 py-12 px-6 bg-background/50 dark:bg-darkBgSecondary border border-dashed dark:border-darkBorder rounded-xl">
          <p className="text-lg text-muted-foreground dark:text-darkTextLight mb-4">No flavors found matching your criteria</p>
          <Button
            variant="outline"
            onClick={resetFilters}
            className="mt-2 font-medium dark:bg-darkFormBg dark:border-darkBorder dark:text-darkTextHeading hover:dark:bg-primaryAccent hover:dark:text-darkTextHeading"
          >
            Clear Filters
          </Button>
        </div>
